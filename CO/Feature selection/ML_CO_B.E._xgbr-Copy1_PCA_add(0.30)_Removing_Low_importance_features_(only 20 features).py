...
df3 = df2.drop(labels=["CO_B.E", 'M-At wt.','M-B.P', 'M-Enth.fus', 'M-Enth.atom', 
                       'M-Enth.vap', 'M-Sp.ht Cap',
                       'M-At.radii', 'M-Group', 'M-Period','M-Elec.Aff', 
                       'At wt.',
                       'Enth.fus', 'Enth.atom', 'Enth.vap', 'Sp.ht Cap',
                       'Period', ], axis=1)
df3
...
average = np.array([0]*20, dtype=np.float64)
feature_importances = []
...
plt.bar(x=range(20), height=np.mean(feature_importances, axis=0), yerr=np.std(feature_importances, axis=0))
plt.xticks(ticks = range(20),labels=X_train.columns, rotation=90)
