...
average = np.array([0]*10, dtype=np.float64)
feature_importances = []
...
plt.bar(x=range(10), height=np.mean(feature_importances, axis=0), yerr=np.std(feature_importances, axis=0))
plt.xticks(ticks = range(10),labels=X_train.columns, rotation=90)
